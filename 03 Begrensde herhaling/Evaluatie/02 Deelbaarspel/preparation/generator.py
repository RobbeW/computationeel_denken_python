...
for test in cases:
    ...
    numbers = []
    for _ in range(3):
        numbers.append( tuple(random.randint(20, 50) for _ in range(1)) )
    
    for nums in numbers:
        num1 = nums[0]
        outputtxt += f"Het getal is {num1}\n"
        
        if random.randint(0,3) != 0: # correcte deler gevonden
            flag = True
            while flag:
                deler = random.randint(1,num1+1)
                if num1 % deler == 0:
                    flag = False

            inputtxt += str(deler)+"\n"
            outputtxt += "Juist!\n"
            if deler != 1 and deler != num1:
                score += 2
            else:
                score += 1
        else:
            ...
            inputtxt += str(deler)+"\n"
            outputtxt += "Dit is geen deler!\n"
        
    outputtxt += f"Score: {score}\n"
