...
ntests = 30
opties = ["J", "F"]
cases_u = [("JJFFJFFJJJ",), ("JJFFJJFFFJJ",), ("JFJFJFJFJFJFJF",), ("JJJJJJJJJJ",), ("JJJJFJJJJFJJJJF",)]
while len(cases_u) < ntests:
    k = random.randint(1,80)
    p = [random.random() for _ in range(len(opties))]
    s = sum(p)
    p = [ i / s for i in p ]
    case = "".join(random.choices(opties, k = k, weights=p ))
    cases_u.append( (case,))
